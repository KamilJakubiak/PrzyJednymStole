// -- Outer fill / stroke globals (temporarily mutated inside buildCloud) --
let outerFill   = 'white';
let outerStroke = '#8f92a4';

// Outer boundary path for each logo shape — used to build group outline layers
const LOGO_OUTER_EL = [
...
// -- Logo shapes: [viewW, viewH, fn(accentColor, textColor) → SVG string] --

const TS = (tc, ty) =>
  tc ? `<text fill="${tc}" style="font-family:Manrope,sans-serif;font-size:48px;font-weight:800;letter-spacing:-2px" transform="translate(60.76 ${ty})">#PrzyJednymStole</text>` : '';
...
// Navy-text logos (0-2) get yellow/orange/cyan; yellow-text logos (3-5) get red/green/purple
const BRAND_NAVY   = ['#fdcd14', '#ff931e', '#09b8e3'];
const BRAND_YELLOW = ['#e30615', '#30a533', '#ab539a'];

function assignBrandColors(pickedLogos) {
  const navy = shuffled([...BRAND_NAVY]);
  const yell = shuffled([...BRAND_YELLOW]);
  let ni = 0, yi = 0;
  return pickedLogos.map(idx => idx < 3 ? navy[ni++] : yell[yi++]);
}

// Expands each logo's outer boundary path by expand_px (canvas-space) and fills it
function buildOutlineGroup(outlineElems, LOGO_SCALE, expand_px, fill) {
  const paths = outlineElems.map(({ logoIdx, tf, lw, lh }) => {
    ...
    const cx = (lw / 2).toFixed(1);
    const cy = (lh / 2).toFixed(1);
    const inner = `translate(${cx},${cy}) scale(${sx},${sy}) translate(-${cx},-${cy})`;
    // Fill injected directly onto the element — <g> fill inheritance breaks in canvas/PNG export
...
function buildCloud(count) {
  ...
  const CS  = Math.round(WD * 0.70);  // horizontal step
  const DS  = Math.round(WD * 0.45);  // diagonal x step
  const DRS = Math.round(HD * 0.43);  // diagonal y step

  // Hex-ring slot positions: 0=center, 1-6=ring
  const HEX = [
    { dx:  0,   dy:    0 },
    { dx:  CS,  dy:    0 },
    { dx:  DS,  dy: -DRS },
  ...
  // Ring slot combos per count — diagonal-only for 2-4 to avoid elongated layouts
  let ringPick;
  switch (count) {
    case 1:  ringPick = []; break;
    case 2:  ringPick = [shuffled([2, 3, 5, 6])[0]]; break;
    case 3:  ringPick = shuffled([[2,6],[3,5],[2,3],[5,6]][Math.floor(Math.random() * 4)]); break;
    case 4:  ringPick = shuffled([[2,3,5],[2,3,6],[3,5,6],[2,5,6]][Math.floor(Math.random() * 4)]); break;
    case 5:  ringPick = shuffled([2, 3, 5, 6]); break;
    default: ringPick = shuffled([1, 2, 3, 4, 5, 6]).slice(0, 5);
  }

  // Ring slots first, center last → center always renders on top
  ...
  const palette = assignBrandColors(pickedLogos);
  const savedFill = outerFill, savedStroke = outerStroke;
  ...
  const pieces = pickedLogos.map((logoIdx, i) => {
    ...
    outerFill = 'none'; outerStroke = 'none';
    let content = contentFn(color, tc);
    outerFill = savedFill; outerStroke = savedStroke;
    // Strip invisible outer shell (fill/stroke both none) from individual pieces
    content = content.replace(/<(?:path|rect)[^>]*fill="none"[^>]*stroke="none"[^>]*\/>/g, '');

    return `<g transform="${tf}">${content}</g>`;
  });

  const inner = `${buildOutlineGroup(outlineElems, LOGO_SCALE, 4.5, '#8f92a4')}
...
// -- Text-to-path for export (Manrope ExtraBold 800, 48px, letter-spacing -2px) --
...
const randomBtn = document.getElementById('count-random');
const ticks     = [...document.querySelectorAll('.slider-ticks span')];

// -- UI --

function getCount() {
  return randomBtn.classList.contains('active')
...
function render() {
  ...
  ticks.forEach((el, i) => el.classList.toggle('active', !isRandom && i + 1 === count));
  logoDiv.innerHTML = buildCloud(count);
}

function onCountRandomToggle() {
  const isRandom = randomBtn.classList.toggle('active');
  slider.classList.toggle('thumb-hidden', isRandom);
  render();
}

// Touching the slider deactivates random mode
...
function replaceTextWithPaths(svg) {
  svg.querySelectorAll('text').forEach(textEl => {
    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    path.setAttribute('fill', textEl.getAttribute('fill') || '#000000');
    const tf = textEl.getAttribute('transform');
    if (tf) path.setAttribute('transform', tf);
    path.setAttribute('d', TEXT_OUTLINE);
...
function downloadSVG() {
  ...
  replaceTextWithPaths(clone);
  const str = '<?xml version="1.0" encoding="UTF-8"?>\n' + new XMLSerializer().serializeToString(clone);
  const url = URL.createObjectURL(new Blob([str], { type: 'image/svg+xml' }));
  Object.assign(document.createElement('a'), { href: url, download: 'logo-composition.svg' }).click();
...
function downloadPNG() {
  ...
  img.onload = () => {
    ...
    Object.assign(document.createElement('a'), { href: canvas.toDataURL('image/png'), download: 'logo-composition.png' }).click();
    URL.revokeObjectURL(url);
  };
  img.src = url;
}
